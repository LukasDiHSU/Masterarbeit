...
if ! require_display_or_print; then
  ...
  for rid in "${ROBOT_IDS[@]}"; do
    echo "cd \"$PROJECT_ROOT\" && AGENT_COUNT=$AGENT_COUNT AGENT_PLATFORM=$AGENT_PLATFORM AGENT_MCP_URL=http://$MCP_CONNECT_HOST:$MCP_PORT/sse python -m agentpackage.architectures.centralized.robot_agent --robot-id $rid --host \"$BROKER_HOST\" --port \"$BROKER_PORT\""
  done
  echo "cd \"$PROJECT_ROOT\" && AGENT_COUNT=$AGENT_COUNT AGENT_PLATFORM=$AGENT_PLATFORM python -m agentpackage.architectures.centralized.master_agent --host \"$BROKER_HOST\" --port \"$BROKER_PORT\""
  exit 1
fi
...
for rid in "${ROBOT_IDS[@]}"; do
  launch_window "Robot $rid" \
    "$SHARED_ENV python -m agentpackage.architectures.centralized.robot_agent --robot-id $rid --host \"$BROKER_HOST\" --port \"$BROKER_PORT\""
done
launch_window "Master Agent" \
  "AGENT_COUNT=$AGENT_COUNT AGENT_WORLD=$AGENT_WORLD AGENT_PLATFORM=$AGENT_PLATFORM python -m agentpackage.architectures.centralized.master_agent --host \"$BROKER_HOST\" --port \"$BROKER_PORT\""
