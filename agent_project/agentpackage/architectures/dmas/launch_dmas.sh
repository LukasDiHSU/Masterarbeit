...
if ! require_display_or_print; then
  ...
  for rid in "${ROBOT_IDS[@]}"; do
    echo "cd \"$PROJECT_ROOT\" && AGENT_COUNT=$AGENT_COUNT AGENT_WORLD=$AGENT_WORLD AGENT_PLATFORM=$AGENT_PLATFORM AGENT_MCP_URL=http://$MCP_CONNECT_HOST:$MCP_PORT/sse python -m agentpackage.architectures.dmas.robot_agent --robot-id $rid --host \"$MESH_HOST\" --base-port \"$MESH_BASE_PORT\""
  done
  echo "cd \"$PROJECT_ROOT\" && AGENT_COUNT=$AGENT_COUNT AGENT_WORLD=$AGENT_WORLD python -m agentpackage.architectures.dmas.mission_cli --host \"$MESH_HOST\" --base-port \"$MESH_BASE_PORT\" --cli-port \"$MESH_CLI_PORT\""
  exit 1
fi

detect_terminal || exit 1
init_experiment_session "dmas"
...
echo "Launching DMAS architecture..."
...
echo "  Agents: $AGENT_COUNT robots + mission CLI (CLI chairs turn-taking)"
...
for ((i=${#ROBOT_IDS[@]}-1; i>=0; i--)); do
  rid="${ROBOT_IDS[$i]}"
  launch_window "DMAS $rid" \
    "$SHARED_ENV python -m agentpackage.architectures.dmas.robot_agent --robot-id $rid --host \"$MESH_HOST\" --base-port \"$MESH_BASE_PORT\""
done
sleep 1
launch_window "Mission CLI" \
  "AGENT_COUNT=$AGENT_COUNT AGENT_WORLD=$AGENT_WORLD AGENT_PLATFORM=$AGENT_PLATFORM python -m agentpackage.architectures.dmas.mission_cli --host \"$MESH_HOST\" --base-port \"$MESH_BASE_PORT\" --cli-port \"$MESH_CLI_PORT\""
