...
if ! require_display_or_print; then
  ...
  for ((i=0; i<AGENT_COUNT; i++)); do
    echo "cd \"$PROJECT_ROOT\" && AGENT_COUNT=$AGENT_COUNT AGENT_WORLD=$AGENT_WORLD AGENT_MCP_URL=http://$MCP_CONNECT_HOST:$MCP_PORT/sse python -m agentpackage.architectures.dmas.robot_agent --robot-id SmallDeliveryRobot_$i --host \"$MESH_HOST\" --base-port \"$MESH_BASE_PORT\""
  done
...
for ((i=AGENT_COUNT-1; i>=0; i--)); do
  launch_window "DMAS SmallDeliveryRobot_$i" \
    "$SHARED_ENV python -m agentpackage.architectures.dmas.robot_agent --robot-id SmallDeliveryRobot_$i --host \"$MESH_HOST\" --base-port \"$MESH_BASE_PORT\""
done
sleep 1
launch_window "Mission CLI" \
  "AGENT_COUNT=$AGENT_COUNT AGENT_WORLD=$AGENT_WORLD python -m agentpackage.architectures.dmas.mission_cli --host \"$MESH_HOST\" --base-port \"$MESH_BASE_PORT\" --cli-port \"$MESH_CLI_PORT\""
