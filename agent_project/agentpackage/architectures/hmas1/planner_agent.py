...
class CentralPlannerAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        prompt = (
            q1_hmas1_planner(fleet=fleet, n=AGENT_COUNT, max_steps=MAX_PLAN_STEPS)
            if is_q1_platform()
            else hmas1_planner(fleet=fleet, n=AGENT_COUNT, max_steps=MAX_PLAN_STEPS)
        )
        super().__init__(
            AgentSpec(
                name=PLANNER_NAME,
    ...
    def _mcp_tools_by_name(self) -> dict:
        names = Q1_PLANNING_MAP_TOOL_NAMES
        return {
            t.name: t
        ...
        *,
        ...
        def _ask(speaker: str) -> tuple[str, str]:
            ...
            reply = self._ask_robot(
                ...
                timeout=mesh_timeout(TURN_TIMEOUT),
            )
        ...
        *,
        ...
        def _one(peer: str) -> tuple[str, str]:
            ...
            reply = self._ask_robot(
                ...
                timeout=mesh_timeout(EXECUTE_TIMEOUT),
            )
            return peer, reply
