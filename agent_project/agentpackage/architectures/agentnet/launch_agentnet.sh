...
# Launches AgentNet (DMAS): MCP + AGENT_COUNT mesh nodes + task CLI.
# Fleet size: Q1 always 3 specialists (navigator, lidar, camera).
# Remroc: AGENT_COUNT / AGENTS / --agents (2|4|6|8, default 4).
# Mission enters at navigator, which chairs turn-taking.
...
if ! require_display_or_print; then
  ...
  echo "cd \"$PROJECT_ROOT\" && AGENT_WORLD=$AGENT_WORLD python -m agentpackage.mcpserver --host \"$MCP_HOST\" --port \"$MCP_PORT\""
  for rid in "${ROBOT_IDS[@]}"; do
    echo "cd \"$PROJECT_ROOT\" && AGENT_COUNT=$AGENT_COUNT AGENT_WORLD=$AGENT_WORLD AGENT_PLATFORM=$AGENT_PLATFORM AGENT_MCP_URL=http://$MCP_CONNECT_HOST:$MCP_PORT/sse python -m agentpackage.architectures.agentnet.net_agent --robot-id $rid --host \"$MESH_HOST\" --base-port \"$MESH_BASE_PORT\""
  done
  echo "cd \"$PROJECT_ROOT\" && AGENT_COUNT=$AGENT_COUNT AGENT_WORLD=$AGENT_WORLD python -m agentpackage.architectures.agentnet.task_cli --host \"$MESH_HOST\" --base-port \"$MESH_BASE_PORT\" --cli-port \"$MESH_CLI_PORT\""
  exit 1
fi

detect_terminal || exit 1
init_experiment_session "agentnet"
...
echo "Launching AGENTNET architecture..."
echo "  MCP bind:    $MCP_HOST:$MCP_PORT"
echo "  MCP connect: $MCP_URL"
echo "  AGENT_WORLD: $AGENT_WORLD"
echo "  Mesh base:   $MESH_HOST:$MESH_BASE_PORT (${ROBOT_IDS[*]})"
echo "  AGENT_PLATFORM: $AGENT_PLATFORM"
echo "  Agents: $AGENT_COUNT DMAS nodes + task CLI (chair: ${ROBOT_IDS[0]})"
...
for ((i=${#ROBOT_IDS[@]}-1; i>=0; i--)); do
  rid="${ROBOT_IDS[$i]}"
  launch_window "Node $rid" \
    "$SHARED_ENV python -m agentpackage.architectures.agentnet.net_agent --robot-id $rid --host \"$MESH_HOST\" --base-port \"$MESH_BASE_PORT\""
done
sleep 1
launch_window "Task CLI" \
  "AGENT_COUNT=$AGENT_COUNT AGENT_WORLD=$AGENT_WORLD AGENT_PLATFORM=$AGENT_PLATFORM python -m agentpackage.architectures.agentnet.task_cli --host \"$MESH_HOST\" --base-port \"$MESH_BASE_PORT\" --cli-port \"$MESH_CLI_PORT\""
