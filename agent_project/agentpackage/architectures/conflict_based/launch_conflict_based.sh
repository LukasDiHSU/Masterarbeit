...
if ! require_display_or_print; then
  ...
  for rid in "${ROBOT_IDS[@]}"; do
    echo "cd \"$PROJECT_ROOT\" && AGENT_COUNT=$AGENT_COUNT AGENT_PLATFORM=$AGENT_PLATFORM AGENT_MCP_URL=http://$MCP_CONNECT_HOST:$MCP_PORT/sse python -m agentpackage.architectures.conflict_based.robot_peer_agent --robot-id $rid --host \"$MESH_HOST\" --base-port \"$MESH_BASE_PORT\""
  done
...
for ((i=${#ROBOT_IDS[@]}-1; i>=0; i--)); do
  rid="${ROBOT_IDS[$i]}"
  launch_window "Peer $rid" \
    "$SHARED_ENV python -m agentpackage.architectures.conflict_based.robot_peer_agent --robot-id $rid --host \"$MESH_HOST\" --base-port \"$MESH_BASE_PORT\""
done
sleep 1
launch_window "Mission CLI" \
  "AGENT_COUNT=$AGENT_COUNT AGENT_WORLD=$AGENT_WORLD AGENT_PLATFORM=$AGENT_PLATFORM python -m agentpackage.architectures.conflict_based.mission_cli --host \"$MESH_HOST\" --base-port \"$MESH_BASE_PORT\" --cli-port \"$MESH_CLI_PORT\""
